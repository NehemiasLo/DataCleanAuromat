...
def load_sample_dataset():
    data = {
        ...
        'Nombre_Completo': ['  juan perez  ', 'Marta Gómez', 'Carlos Ruiz', 'ana logan', 'ana logan', 'luis Fernandez', 'Sofía herrera', 'Pedro Almodovar'],
        ...
        'Fecha_Registro': ['2026/01/15', '16-01-2026', '2026.01.17', '2026/01/18', '2026/01/18', '2026-01-19', np.nan, '2026-01-21']
    }
...
st.markdown("<p style='text-align: center; color: #6B7280; font-size: 0.9rem; margin: 1rem 0 0.5rem 0;'>¿No tienes un archivo a la mano para probar?</p>", unsafe_allowed_html=True)
...
if uploaded_file is not None:
    ...
    st.info("Se ha cargado el dataset de prueba con errores comunes (duplicados, nulos, problemas de escritura, formatos monetarios inconsistentes y valores extremos).")
...
if df is not None:
    ...
    st.markdown("<p style='color:#6B7280; font-size:0.9rem; margin-bottom:1.5rem;'>El sistema ha analizado tu archivo y encontró los siguientes detalles estructurales:</p>", unsafe_allowed_html=True)
    ...
    with col2:
        ...
        st.metric(label="Celdas Vacías (Nulos)", value=total_nulos, delta=-total_nulos if total_nulos > 0 else 0, delta_color="inverse")
    st.markdown('</div>', unsafe_allowed_html=True)
    ...
    st.markdown("<p style='color:#6B7280; font-size:0.95rem; margin-bottom:1.5rem;'>Elige qué operaciones automáticas deseas aplicar al dataset (están todas activadas por defecto para una limpieza óptima):</p>", unsafe_allowed_html=True)
    
    col_opt1, col_opt2 = st.columns(2)
    with col_opt1:
        clean_duplicates = st.checkbox("Eliminar filas repetidas/duplicadas", value=True, help="Remueve copias idénticas exactas en los registros.")
            ...
            if clean_outliers:
                for col in df_cleaned.select_dtypes(include=np.number).columns:
                    ...
                    if negative_count > 0:
                        ...
                        logs.append(f"En **'{col}'** se corrigieron **{negative_count} valores negativos** incoherentes.")
                ...
                for col in df_cleaned.select_dtypes(include=['object']).columns:
                    if df_cleaned[col].notna().any():
                        ...
                        if 'ciudad' in col.lower() or 'municipio' in col.lower() or 'departamento' in col.lower():
                            ...
                            bogota_variations = ['Bogotá', 'Bogota', 'bogota', 'BOGOTA', 'Bogotá D.C.', 'Bogota D.C.']
    ...
    if 'df_cleaned' in st.session_state:
        ...
        tab_comparacion, tab_detalles, tab_visualizar = st.tabs(["📊 Comparativa de Datos", "📋 Historial de Correcciones", "📈 Exploración Gráfica"])
        
        with tab_comparacion:
            col_antes, col_despues = st.columns(2)
            with col_antes:
                st.markdown("<p style='color: #EF4444; font-weight: 600;'>❌ Dataset Original (Con errores):</p>", unsafe_allowed_html=True)
                st.dataframe(df.head(10), use_container_width=True)
                st.caption(f"Filas originales: {df.shape[0]} | Columnas: {df.shape[1]}")
            with col_despues:
                st.markdown("<p style='color: #10B981; font-weight: 600;'>✅ Dataset Optimizado (Limpio):</p>", unsafe_allowed_html=True)
                st.dataframe(df_final.head(10), use_container_width=True)
                st.caption(f"Filas limpias: {df_final.shape[0]} | Columnas: {df_final.shape[1]}")

        with tab_detalles:
            st.markdown("<p style='color:#374151; font-weight: 500;'>Detalle de acciones tomadas de forma automática:</p>", unsafe_allowed_html=True)
            if logs_finales:
                for log in logs_finales:
                    st.markdown(f"<div style='color:#4B5563; font-size:0.95rem; margin-bottom:0.5rem; padding-left:0.75rem; border-left:3px solid #1E3A8A;'>{log}</div>", unsafe_allowed_html=True)
            else:
                st.markdown("<p style='color:#6B7280;'>No se requirieron modificaciones. El dataset cumple con todos los estándares de producción.</p>", unsafe_allowed_html=True)
        ...
        with tab_visualizar:
            ...
            if not numerical_cols.empty or not categorical_cols.empty:
                st.markdown("<p style='color:#6B7280; font-size:0.9rem; margin-bottom:1.5rem;'>Gráficos generados para entender las variables del archivo limpio:</p>", unsafe_allowed_html=True)
                
                if not numerical_cols.empty:
                    for col in numerical_cols:
                        fig, ax = plt.subplots(figsize=(7, 2.2), facecolor='none')
                        sns.histplot(df_final[col], kde=True, ax=ax, color=brand_color, edgecolor='#ffffff', alpha=0.8)
                        ...
                        ax.set_title(f'Distribución de {col}', color='#111827', fontsize=10, pad=8, weight='semibold')
                        st.pyplot(fig)
                        plt.close(fig)
                ...
                if not categorical_cols.empty:
                    for col in categorical_cols:
                        ...
                        if not top_10.empty:
                            ...
                            sns.barplot(x=top_10.index, y=top_10.values, palette=sns.color_palette("Blues_r", len(top_10)), ax=ax)
                            ax.set_facecolor('none')
                            ax.spines['top'].set_visible(False)
                            ax.spines['right'].set_visible(False)
                            ax.spines['left'].set_color('#D1D5DB')
                            ax.spines['bottom'].set_color('#D1D5DB')
                            ax.tick_params(colors='#4B5563', labelsize=8)
                            ax.set_title(f'Top 10 Valores de la columna {col}', color='#111827', fontsize=10, pad=8, weight='semibold')
                            plt.xticks(rotation=45, ha='right')
                            st.pyplot(fig)
                            plt.close(fig)
            else:
                st.markdown("<p style='color:#6B7280;'>No hay columnas numéricas ni categóricas suficientes para estructurar visualizaciones.</p>", unsafe_allowed_html=True)

        # --- Descarga directa del archivo pulido ---
        st.markdown("<hr>", unsafe_allowed_html=True)
        st.markdown("<p style='text-align: center; color:#374151; font-weight: 500;'>¿Todo listo? Descarga tu archivo purificado aquí:</p>", unsafe_allowed_html=True)
    ...
    st.markdown(f"""
        <div style='padding:1.5rem; background-color:#FEF2F2; border:1px solid #FCA5A5; border-radius:8px; color:#991B1B;'>
